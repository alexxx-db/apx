...
<Callout type="info">
**Note:** apx does not use a `components.json` file. Component registries are configured in `pyproject.toml` under the `[tool.apx.ui.registries]` section. The [shadcn/ui](https://ui.shadcn.com/) registry is used as the default, so there's no need to add it manually.
</Callout>
